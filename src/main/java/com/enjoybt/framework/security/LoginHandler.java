...
public class LoginHandler implements AuthenticationSuccessHandler, AuthenticationFailureHandler, UserDetailsService, AuthenticationManager  {
	...
	public Authentication authenticate(final Authentication authentication) throws AuthenticationException {
        ...
        if(!user.isAccountNonLocked()){
        	throw new LockedException(Constants.LOGIN_FAILURE_OVER_ATTEMPT_COUNT+":"+userId);
        }
       
        if(!encoder.matches(pwd, user.getPassword())){
            sqlSession.update("security.addLoginCnt", userId);

            throw new BadCredentialsException(Constants.LOGIN_FAILURE_NO_MATCH_PASSWORD+":"+userId);
        }
